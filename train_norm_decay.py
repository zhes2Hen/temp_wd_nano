# train with a simple norm schedule (or norm decay)
"""
...
# I/O
out_dir = 'out'
suffix = '_norm_decay'  # filename suffix
eval_interval = 2000
log_interval = 1
eval_iters = 200
...
begin_norm_decay_step = 2000 # the iteration to add a sphere projected regularization
norm_decay_percent = 0.7 # params norm (except LayerNorm) will decay to 70%
# DDP settings
...
def create_sphere_constrained_adam(
    ...
    """
    为 model 构造一个 Adam 优化器，但在每次 step 后
    会对所有 p.dim()>=2 的参数做：
        p <- c_dict[name] * p / ||p||
    以保证这些张量的 Frobenius 范数恒等于 c_dict[name]
    """
    ...
    def sphere_step(closure=None):
        ...
        # sphere proj
        if iter_num>begin_norm_decay_step:

            _norm_decay_ratio=(iter_num-begin_norm_decay_step)/(max_iters-begin_norm_decay_step)
            _norm_coeff=norm_decay_percent+(1+math.cos(math.pi*_norm_decay_ratio))*(1-norm_decay_percent)/2
            ...
            with torch.no_grad():
                for name, param in decay:
                    ...
                    if norm > 0:
                        c = begin_regularization_c_dict[name]
...
while True:
    ...
    if iter_num==begin_norm_decay_step:
        begin_regularization_c_dict=get_c_dict()
